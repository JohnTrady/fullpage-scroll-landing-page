(() => {
    ...
    function initSliders() {
        if (document.querySelector(".swiper")) {
            let pageSlider = new core(".swiper", {
                modules: [ Pagination, Mousewheel, Keyboard, freeMode, Scrollbar, Parallax ],
                direction: "vertical",
                observer: true,
                observeParents: true,
    ...
    function circleDiagramInit() {
        if (document.querySelectorAll("[data-dc]").length > 0) {
            const circleDiagramValue = document.querySelectorAll(".circle-value");
            const calculateCircleDiagram = () => {
                document.querySelectorAll("[data-dc]").forEach(((circleItem, index) => {
                    let circleValue = circleItem.getAttribute("data-dc");
                    circleItem.style.background = `conic-gradient(#1C1F25 0.00% ${circleValue}%,rgba(28, 31, 37, 0.1) 0.00% ${100 - circleValue}%)`;
                    circleDiagramValue[index].textContent = `${circleValue}%`;
                }));
            };
            calculateCircleDiagram();
        }
    }
    circleDiagramInit();
    function ringDiagramInit() {
        if (document.querySelectorAll("[data-dr]").length > 0) {
            const ringDiagramValue = document.querySelectorAll(".ring-value span");
            document.querySelectorAll("[data-dr]").forEach(((ringItem, index) => {
                const radius = ringItem.r.baseVal.value;
                const circumference = 2 * Math.PI * radius;
                let ringValue = ringItem.getAttribute("data-dr");
                ringItem.style.strokeDasharray = `${circumference} ${circumference}`;
                ringItem.style.strokeDashoffset = circumference;
                const setProgress = percent => {
                    const offset = circumference - percent / 100 * circumference;
                    ringItem.style.strokeDashoffset = offset;
                    ringDiagramValue[index].textContent = ringValue;
                };
                setProgress(ringValue);
            }));
        }
    }
    ringDiagramInit();
